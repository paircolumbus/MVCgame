...
class Player
  ...
  def go direction
    ...
    if @location.blocked? direction
      messages << "The way is blocked by the #{@location.items[direction].name}."
      return
    end
  ...
  def can_reach_item? item
    return @location.contains?(item) || @inventory.member?(item) || (@weapon == item)
  end
